...
def get_top_rated_books(
    min_rating: int = Query(4, ge=0, le=5, description="Minimum rating to filter top-rated books"),
    db: Session = Depends(get_db)
):
    """
    ...
    """
    books = db.query(Book).filter(Book.rating >= min_rating).order_by(Book.rating.desc()).all()
    return books
...
def get_books_in_price_range(
    ...
    max_price: float = Query(1000.0, ge=0.0, description="Maximum price to filter books"),
    db: Session = Depends(get_db)
):
    """
    ...
    """
    books = db.query(Book).filter(Book.price >= min_price, Book.price <= max_price).order_by(Book.price).all()
    return books